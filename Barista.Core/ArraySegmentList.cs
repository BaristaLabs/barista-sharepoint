namespace Barista
{
      ...
      where T : IEquatable<T>
  {
    ...
    public int IndexOf(T item)
    {
      ...
      foreach (var currentSegment in m_segments)
      {
        var offset = currentSegment.Offset;

        for (var j = 0; j < currentSegment.Count; j++)
        {
    ...
    public T[] ToArrayData(int startIndex, int length)
    {
      ...
      for (var i = startSegmentIndex; i < m_segments.Count; i++)
      {
        ...
        int len = Math.Min(currentSegment.Count - @from, length - total);
    ...
    public int CopyTo(T[] to, int srcIndex, int toIndex, int length)
    {
      ...
      int thisCopied = Math.Min(offsetSegment.Count - thisOffset + offsetSegment.Offset, length - copied);
