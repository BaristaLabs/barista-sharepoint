namespace Barista
{
    ...
    public static class ADHelper
    {
        ...
        public static ADUser GetADUserByDistinguishedName(string distinguishedName, string ldapPath)
        {
            if (ldapPath.IsNullOrWhiteSpace())
                ldapPath = ADHelper.LdapPath;
            ...
            using (var ctx = new ADContext(ldapRoot))
            {
                ...
                result = (from usr in ctx.Users
                          where usr.DistinguishedName == distinguishedName
                          select usr).ToList().FirstOrDefault();
            }
        ...
        public static ADGroup GetADGroupByDistinguishedName(string distinguishedName, string ldapPath)
        {
            if (ldapPath.IsNullOrWhiteSpace())
                ldapPath = ADHelper.LdapPath;
            ...
            using (var ctx = new ADContext(ldapRoot))
            {
                ...
                result = (from grp in ctx.Groups
                          where grp.DistinguishedName == distinguishedName
                          select grp).ToList().FirstOrDefault();
            }
          ...
          PrincipalType principalType, string ldapPath)
        {
            ...
            using (var ctx = new ADContext(ldapRoot))
            {
                ...
                {
                    var users = ctx.Users.Where(usr => usr.FirstName == "*" + searchText + "*" ||
        ...
        public static IEnumerable<DirectoryEntity> SearchAllDirectoryEntitiesByLogonAndEmail(string searchText, int maxResults, PrincipalType principalType, string ldapPath)
        {
            if (ldapPath.IsNullOrWhiteSpace())
                ldapPath = ADHelper.LdapPath;
            ...
            using (var ctx = new ADContext(ldapRoot))
            {
                ...
                if ((principalType & PrincipalType.User) == PrincipalType.User)
                {
                    var users = ctx.Users.Where(usr => usr.Email == "*" + searchText + "*" ||
                                                       usr.PreWin2kLogonName == "*" + searchText + "*");
        ...
        {
            return SearchAllDirectoryEntities(searchText, maxResults, PrincipalType.User, ldapPath)
                .OfType<ADUser>()
                .ToList();
        }

        public static IEnumerable<ADUser> SearchAllUsersByLogonAndEmail(string searchText, int maxResults, string ldapPath)
        {
            return SearchAllDirectoryEntitiesByLogonAndEmail(searchText, maxResults, PrincipalType.User, ldapPath)
                .OfType<ADUser>()
                .ToList();
        }
