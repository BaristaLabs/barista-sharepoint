namespace Jurassic.Compiler
{
 ...
 : System.Runtime.Serialization.ISerializable
#endif
  {
    ...
    private Type m_paramArrayElementType;
    ...
    private void Init()
    {
      ...
      for (int i = 0; i < parameters.Length; i++)
      {
        ...
        if (i == parameters.Length - 1 && Attribute.IsDefined(parameters[i], typeof(ParamArrayAttribute)))
        {
    ...
    public int RequiredParameterCount
    {
      get
      {
        if (this.m_initialized == false)
          Init();
        return this.m_requiredParameterCount;
      }
    }
    ...
    public int OptionalParameterCount
    {
      get
      {
        if (this.m_initialized == false)
          Init();
        return this.m_optionalParameterCount;
      }
    }
    ...
    public bool HasParamArray
    {
      get
      {
        if (this.m_initialized == false)
          Init();
        return this.m_paramArrayElementType != null;
      }
    }
    ...
    private Type ParamArrayElementType
    {
      get
      {
        if (this.m_initialized == false)
          Init();
        return this.m_paramArrayElementType;
      }
    }
    ...
    public virtual IEnumerable<BinderArgument> GetArguments(int argumentCount)
    {
      ...
      if (this.Method is MethodInfo && this.Method.IsStatic == false)
        yield return new BinderArgument(BinderArgumentSource.ThisValue, this.DeclaringType, -1);
  ...
  internal class BinderArgument
  {
    ...
    internal BinderArgument(BinderArgumentSource source, Type type, int index)
    {
      this.Source = source;
    ...
    {
      get { return this.m_parameterInfo != null && (this.m_parameterInfo.Attributes & ParameterAttributes.HasDefault) != 0; }
    }
    ...
    public object DefaultValue
    {
      get
      {
        if (this.m_parameterInfo == null || this.HasDefaultValue == false)
          return null;
        var attribute = GetCustomAttribute<DefaultParameterValueAttribute>();
        if (attribute == null)
          throw new InvalidOperationException(string.Format("Expected [DefaultParameterValue] on parameter '{0}'.", this.m_parameterInfo.Name));
        if (attribute.Value == null && this.Type.IsValueType)
          throw new InvalidOperationException(string.Format("Null is not a valid default value for parameter '{0}'.", this.m_parameterInfo.Name));
        if (attribute.Value != null && attribute.Value.GetType() != this.Type)
          throw new InvalidOperationException(string.Format("Default value for parameter '{0}' should be '{1}'.", this.m_parameterInfo.Name, this.Type));
    ...
    {
      if (this.m_parameterInfo == null)
        return default(T);
      return (T)Attribute.GetCustomAttribute(this.m_parameterInfo, typeof(T));
    }
  }
