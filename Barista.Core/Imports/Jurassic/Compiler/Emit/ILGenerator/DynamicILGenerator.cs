namespace Barista.Jurassic.Compiler
{
  ...
  {
    private readonly System.Reflection.Emit.DynamicMethod m_dynamicMethod;
    private readonly System.Reflection.Emit.DynamicILInfo m_dynamicILInfo;
...
#pragma warning disable 0649
    private readonly Stack<VESType> m_operands;
#pragma warning restore 0649
    private bool m_stackIsIndeterminate;

    // All of the local variables defined within this method.
    private readonly List<DynamicILLocalVariable> m_localVariables;

    // The local variable signature blob.
    private readonly System.Reflection.Emit.SignatureHelper m_signatureHelper;

    // All the labels defined within the method.
    private readonly List<DynamicILLabel> m_labels;
    ...
    private readonly List<Fixup> m_fixups;
    ...
    public DynamicILGenerator(System.Reflection.Emit.DynamicMethod dynamicMethod)
    {
      ...
      this.m_dynamicMethod = dynamicMethod;
      this.m_dynamicILInfo = dynamicMethod.GetDynamicILInfo();
      this.m_bytes = new byte[100];
      this.m_localVariables = new List<DynamicILLocalVariable>();
      this.m_signatureHelper = System.Reflection.Emit.SignatureHelper.GetLocalVarSigHelper(null);
      this.m_labels = new List<DynamicILLabel>();
      this.m_fixups = new List<Fixup>();

#if DEBUG
      this.m_operands = new Stack<VESType>();
#endif
    }
    ...
    public override unsafe void Complete()
    {
      ...
      if (this.m_activeExceptionRegions != null && this.m_activeExceptionRegions.Count > 0)
        throw new InvalidOperationException("The current method contains unclosed exception blocks.");

      Return();
      FixLabels();
      fixed (byte* bytes = this.m_bytes)
        this.m_dynamicILInfo.SetCode(bytes, this.m_offset, this.m_maxStackSize);
      this.m_dynamicILInfo.SetLocalSignature(this.LocalSignature);

      if (this.m_exceptionRegions == null || this.m_exceptionRegions.Count <= 0)
        return;

      // Count the number of exception clauses.
      int clauseCount = this.m_exceptionRegions.Sum(exceptionRegion => exceptionRegion.Clauses.Count);
      ...
      foreach (var exceptionRegion in this.m_exceptionRegions)
      {
      ...
      writer.Flush();
      this.m_dynamicILInfo.SetExceptions(exceptionBytes);
    }
    ...
    {
      Array.Resize(ref this.m_bytes, this.m_bytes.Length * 2);
    }
    ...
    private void EmitInt16(int value)
    {
      ...
      this.m_bytes[offset++] = (byte)value;
      this.m_bytes[offset++] = (byte)(value >> 8);
    ...
    private void EmitInt32(int value)
    {
      ...
      this.m_bytes[offset++] = (byte)value;
      this.m_bytes[offset++] = (byte)(value >> 8);
      this.m_bytes[offset++] = (byte)(value >> 16);
      this.m_bytes[offset++] = (byte)(value >> 24);
      this.m_offset = offset;
    }
    ...
    private void EmitInt64(long value)
    {
      ...
      this.m_bytes[offset++] = (byte)(value >> 16);
      this.m_bytes[offset++] = (byte)(value >> 24);
      this.m_bytes[offset++] = (byte)(value >> 32);
      this.m_bytes[offset++] = (byte)(value >> 40);
      this.m_bytes[offset++] = (byte)(value >> 48);
      this.m_bytes[offset++] = (byte)(value >> 56);
      this.m_offset = offset;
    }
    ...
    private void Emit1ByteOpCode(byte opCode, int popCount, int pushCount)
    {
      ...
      if (this.m_offset + instructionSize >= this.m_bytes.Length)
        EnlargeArray(instructionSize);

      // Emit the instruction bytes.
      this.m_bytes[this.m_offset++] = opCode;

      // Update the stack.
      if (this.m_stackSize < popCount)
        throw new InvalidOperationException("Stack underflow");
      this.m_stackSize += pushCount - popCount;
      this.m_maxStackSize = Math.Max(this.m_stackSize, this.m_maxStackSize);
    }
    ...
    private void Emit1ByteOpCodeInt8(byte opCode, int popCount, int pushCount, int emitInt8)
    {
      ...
      // Emit the instruction bytes.
      this.m_bytes[this.m_offset++] = opCode;
      this.m_bytes[this.m_offset++] = (byte)emitInt8;
    ...
    private void Emit2ByteOpCode(byte opCode1, byte opCode2, int popCount, int pushCount)
    {
      ...
      // Emit the instruction bytes.
      this.m_bytes[this.m_offset++] = opCode1;
      this.m_bytes[this.m_offset++] = opCode2;
    ...
    {
      return this.m_dynamicILInfo.GetTokenFor(type.TypeHandle);
    }
    ...
    {
      return this.m_dynamicILInfo.GetTokenFor(field.FieldHandle);
    }
    ...
    {
      return this.m_dynamicILInfo.GetTokenFor(str);
    }
    ...
    {
      get { return this.m_maxStackSize; }
    }
    ...
    {
      get { return this.m_stackIsIndeterminate; }
    }
...
#if DEBUG
      PushStackOperand(this.m_operands.Peek());
#endif
    }
    ...
    private void PopStackOperands(params VESType[] expectedTypes)
    {
      ...
      for (int i = 0; i < expectedTypes.Length; i++)
        actualTypes.Add(this.m_operands.Pop());
      actualTypes.Reverse();
      for (int i = 0; i < expectedTypes.Length; i++)
    ...
    {
      this.m_operands.Push(type);
      if (this.m_operands.Count != this.m_stackSize)
    ...
    {
      this.m_stackSize = operandTypes.Length;
      this.m_maxStackSize = Math.Max(this.m_stackSize, this.m_maxStackSize);
#if DEBUG
      this.m_operands.Clear();
      foreach (var operandType in operandTypes)
        this.m_operands.Push(operandType);
#endif
    }
    ...
    private void CheckArithmeticOperands(ArithmeticOperator @operator)
    {
      ...
      var right = this.m_operands.Pop();
      var left = this.m_operands.Pop();
    ...
    private void UnconditionalBranch()
    {
      ...
      this.m_stackIsIndeterminate = true;
    }
    ...
    {
      var result = new DynamicILLabel(this, this.m_labels.Count);
      this.m_labels.Add(result);
      return result;
    }
    ...
    public override void DefineLabelPosition(ILLabel label)
    {
      ...
      if (label2.EvaluationStack != null)
      {
        ...
        if (this.m_stackIsIndeterminate == false)
        {
          ...
          var currentStack = this.m_operands.ToArray();
          ...
          if (previousStack.Where((t, i) => t != currentStack[i]).Any())
          {
            throw new InvalidOperationException(string.Format("Stack mismatch from a previous branch.  Expected: '{0}' but was: '{1}'",
                                                              StringHelpers.Join(", ", previousStack), StringHelpers.Join(", ", currentStack)));
          }
        ...
        else
        {
          ...
          this.m_stackSize = previousStack.Length;
          this.m_operands.Clear();
          for (int i = previousStack.Length - 1; i >= 0; i--)
            this.m_operands.Push(previousStack[i]);
    ...
    {
      foreach (var fix in this.m_fixups)
      {
    ...
    private void BranchCore(ILLabel label, byte opCode)
    {
      ...
      // Emit the label.
      EmitLabel(label, this.m_offset + 4);
    ...
    private void BranchCore(ILLabel label, byte opCode, ComparisonOperator @operator)
    {
      ...
      CheckComparisonOperands(@operator, true);
    ...
    private void EmitLabel(ILLabel label, int startOfNextInstruction)
    {
      ...
      // Enlarge the array if necessary.
      if (this.m_offset + 4 >= this.m_bytes.Length)
        EnlargeArray(4);
      ...
      else
      {
        ...
        EmitInt32(0);
        this.m_fixups.Add(new Fixup { Position = this.m_offset - 4, Length = 4, StartOfNextInstruction = startOfNextInstruction, Label = label2 });
      }
      ...
      if (label2.EvaluationStack == null)
      {
        ...
        label2.EvaluationStack = this.m_operands.ToArray();
      }
      else
      {
    ...
    public override void Return()
    {
      ...
      int popCount = this.m_dynamicMethod.ReturnType == typeof(void) ? 0 : 1;
      ...
      if (this.m_stackIsIndeterminate == false)
      {
        ...
        if (popCount > 0)
          PopStackOperands(ToVESType(this.m_dynamicMethod.ReturnType));
        if (this.m_stackSize != 0)
        {
#if DEBUG
          throw new InvalidOperationException(string.Format("The evaluation stack should be empty.  Types still on stack: {0}.", StringHelpers.Join(", ", this.m_operands)));
#else
    ...
    public override void Switch(ILLabel[] labels)
    {
      ...
      int startOfNextInstruction = this.m_offset + instructionSize;
    ...
    {
      get { return this.m_signatureHelper.GetSignature(); }
    }
    ...
    {
      var result = new DynamicILLocalVariable(this, this.m_localVariables.Count, type, name);
      this.m_localVariables.Add(result);
      this.m_signatureHelper.AddArgument(type, false);
      return result;
    }
    ...
    public override void LoadArgument(int argumentIndex)
    {
      ...
      PushStackOperand(ToVESType(this.m_dynamicMethod.GetParameters()[argumentIndex].ParameterType));
    }
    ...
    public override void StoreArgument(int argumentIndex)
    {
      ...
      PopStackOperands(ToVESType(this.m_dynamicMethod.GetParameters()[argumentIndex].ParameterType));
    }
    ...
    public override void CompareEqual()
    {
      ...
      CheckComparisonOperands(ComparisonOperator.Equal, false);
    }
    ...
    public override void CompareGreaterThan()
    {
      ...
      CheckComparisonOperands(ComparisonOperator.GreaterThan, false);
    }
    ...
    public override void CompareGreaterThanUnsigned()
    {
      ...
      CheckComparisonOperands(ComparisonOperator.GreaterThanUnsigned, false);
    }
    ...
    public override void CompareLessThan()
    {
      ...
      CheckComparisonOperands(ComparisonOperator.LessThan, false);
    }
    ...
    public override void CompareLessThanUnsigned()
    {
      ...
      CheckComparisonOperands(ComparisonOperator.LessThanUnsigned, false);
    }
...
#if DEBUG
      var topOfStack = this.m_operands.Peek();
    ...
    public override void NewObject(System.Reflection.ConstructorInfo constructor)
    {
      ...
      operandTypes.AddRange(parameters.Select(parameter => ToVESType(parameter.ParameterType)));
    ...
    public override void BeginExceptionBlock()
    {
      ...
      if (this.m_activeExceptionRegions == null)
        this.m_activeExceptionRegions = new Stack<ExceptionRegion>();
      ...
      var region = new ExceptionRegion
        {
          ...
          Clauses = new List<ExceptionClause>(3)
        };

      // Push the exception region to the stack.
      this.m_activeExceptionRegions.Push(region);
    }
    ...
    public override void EndExceptionBlock()
    {
      ...
      var exceptionRegion = this.m_activeExceptionRegions.Pop();
      ...
      if (this.m_exceptionRegions == null)
        this.m_exceptionRegions = new List<ExceptionRegion>();
      this.m_exceptionRegions.Add(exceptionRegion);
    }
    ...
    public override void BeginCatchBlock(Type exceptionType)
    {
      ...
      var exceptionRegion = this.m_activeExceptionRegions.Peek();

      // Check there isn't already a catch block with the same type.
      if (exceptionRegion.Clauses.Any(clause => clause.Type == ExceptionClauseType.Catch && clause.CatchToken == exceptionTypeToken))
      {
      ...
      exceptionRegion.Clauses.Add(new ExceptionClause { Type = ExceptionClauseType.Catch, ILStart = this.m_offset, CatchToken = exceptionTypeToken });
    ...
    private void BeginExceptionBlock(ExceptionClauseType type)
    {
      ...
      if (type != ExceptionClauseType.Filter)
      {
        ...
        if (exceptionRegion.Clauses.Any(clause => clause.Type == type))
        {
      ...
      // Add a new clause.
      exceptionRegion.Clauses.Add(new ExceptionClause { Type = type, ILStart = this.m_offset });
    ...
    public override void EndFilter()
    {
      ...
      latestClause.FilterHandlerStart = this.m_offset;
    ...
    private void EndCurrentClause(ExceptionRegion exceptionRegion)
    {
      if (exceptionRegion.Clauses.Count == 0)
      {
        ...
        exceptionRegion.TryLength = this.m_offset - exceptionRegion.Start;
      }
      else
      {
        var latestClause = exceptionRegion.Clauses[exceptionRegion.Clauses.Count - 1];
        ...
        latestClause.ILLength = this.m_offset - latestClause.ILStart;
      }
    }
