namespace Barista.Jurassic.Compiler
{
  ...
  internal abstract class ILGenerator
  {
    ...
    private List<ILLocalVariable> m_temporaryVariables;
    ...
    public ILLocalVariable CreateTemporaryVariable(Type type)
    {
      ...
      if (this.m_temporaryVariables != null)
      {
        for (int i = 0; i < this.m_temporaryVariables.Count; i++)
        {
          if (this.m_temporaryVariables[i].Type == type)
          {
            var result = this.m_temporaryVariables[i];
            this.m_temporaryVariables.RemoveAt(i);
            return result;
          }
        }
      }
    ...
    public void ReleaseTemporaryVariable(ILLocalVariable temporaryVariable)
    {
      ...
      if (this.m_temporaryVariables == null)
        this.m_temporaryVariables = new List<ILLocalVariable>();
      this.m_temporaryVariables.Add(temporaryVariable);
    }
