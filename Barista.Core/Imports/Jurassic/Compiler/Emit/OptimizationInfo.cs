namespace Barista.Jurassic.Compiler
{
  ...
  internal class OptimizationInfo
  {
    ...
    private Dictionary<RegularExpressionLiteral, ILLocalVariable> m_regularExpressionVariables;
    ...
    public ILLocalVariable GetRegExpVariable(ILGenerator generator, RegularExpressionLiteral literal)
    {
      ...
      if (this.m_regularExpressionVariables == null)
        this.m_regularExpressionVariables = new Dictionary<RegularExpressionLiteral, ILLocalVariable>();

      // Check if the literal already exists in the dictionary.
      ILLocalVariable variable;
      if (this.m_regularExpressionVariables.TryGetValue(literal, out variable) == false)
      {
        // The literal does not exist - add it.
        variable = generator.DeclareVariable(typeof(Library.RegExpInstance), null);
        this.m_regularExpressionVariables.Add(literal, variable);
      }
      return variable;
    }
    ...
    private readonly Stack<BreakOrContinueInfo> m_breakOrContinueStack = new Stack<BreakOrContinueInfo>();
    ...
    public void PushBreakOrContinueInfo(IList<string> labelNames, ILLabel breakTarget, ILLabel continueTarget, bool labelledOnly)
    {
      ...
      this.m_breakOrContinueStack.Push(new BreakOrContinueInfo
        {
    ...
    {
      this.m_breakOrContinueStack.Pop();
    }
    ...
    {
      get { return this.m_breakOrContinueStack.Count; }
    }
    ...
    private int GetBreakOrContinueLabelDepth(ILLabel label)
    {
      ...
      int depth = this.m_breakOrContinueStack.Count - 1;
      foreach (var info in this.m_breakOrContinueStack)
      {
    ...
    public void EmitLongJump(ILGenerator generator, ILLabel targetLabel)
    {
      if (this.LongJumpCallback == null)
      {
        ...
        if (this.InsideTryCatchOrFinally)
