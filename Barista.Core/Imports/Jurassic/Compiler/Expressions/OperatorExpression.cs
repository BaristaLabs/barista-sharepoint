namespace Barista.Jurassic.Compiler
{
  ...
  {
    private readonly Expression[] m_operands;
    ...
    protected OperatorExpression(Operator @operator)
    {
      ...
      this.m_operands = new Expression[@operator.Arity];
    }
    ...
    public Expression GetOperand(int index)
    {
      ...
      var result = this.m_operands[index];
      var expression = result as GroupingExpression;
    ...
    {
      if (this.OperandCount >= this.m_operands.Length)
        throw new InvalidOperationException("Too many operands.");
      this.m_operands[this.OperandCount] = operand;
      this.OperandCount++;
    }
    ...
    public Expression Pop()
    {
      ...
      this.OperandCount--;
      return this.m_operands[this.OperandCount];
    }
    ...
    public bool AcceptingOperands
    {
      get
      {
        if (this.SecondTokenEncountered && this.Operator.HasSecondaryRHSOperand == false)
          return false;
    ...
    {
      get { return this.OperandCount == 0 ? null : this.m_operands[this.OperandCount - 1] as OperatorExpression; }
    }
    ...
    public override IEnumerable<AstNode> ChildNodes
    {
      get
      {
        for (int i = 0; i < this.OperandCount; i++)
          yield return this.m_operands[i];
      }
    }
    ...
    public override string ToString()
    {
      ...
      {
        result.Append(this.OperandCount > i ? this.m_operands[i].ToString() : string.Empty);
        i++;
