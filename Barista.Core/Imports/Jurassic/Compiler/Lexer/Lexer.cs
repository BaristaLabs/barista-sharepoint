namespace Barista.Jurassic.Compiler
{
  ...
  internal class Lexer : IDisposable
  {
    ...
    private readonly ScriptSource m_source;
    private readonly TextReader m_reader;
    private int m_lineNumber, m_columnNumber;
    ...
    public Lexer(ScriptEngine engine, ScriptSource source)
    {
      ...
      this.m_reader = source.GetReader();
      this.m_lineNumber = 1;
    ...
    private Token m_lastSignificantToken;
    ...
    public Token NextToken()
    {
      ...
      if (IsPunctuatorStartChar(c1))
      {
        ...
        this.m_lastSignificantToken = ReadPunctuator(c1);
        return this.m_lastSignificantToken;
      }
      ...
      if (IsIdentifierStartChar(c1))
      {
        ...
        this.m_lastSignificantToken = ReadIdentifier(c1);
        return this.m_lastSignificantToken;
      }

      if (IsStringLiteralStartChar(c1))
      {
        // String literal.
        this.m_lastSignificantToken = ReadStringLiteral(c1);
        return this.m_lastSignificantToken;
      }

      if (IsNumericLiteralStartChar(c1))
      {
        // Number literal.
        this.m_lastSignificantToken = ReadNumericLiteral(c1);
        return this.m_lastSignificantToken;
      }
      ...
      {
        // Line Terminator.
        this.m_lastSignificantToken = ReadLineTerminator(c1);
        return this.m_lastSignificantToken;
      }
      ...
      if (c1 == '/')
      {
        ...
        this.m_lastSignificantToken = ReadDivideCommentOrRegularExpression();
        return this.m_lastSignificantToken;
      }
      ...
      {
        // End of input.
        this.m_lastSignificantToken = null;
        return null;
      }

      throw new JavaScriptException(this.m_engine, "SyntaxError", string.Format("Unexpected character '{0}'.", (char)c1), this.m_lineNumber, this.Source.Path);
    }
    ...
    private Token ReadIdentifier(int firstChar)
    {
      ...
      if (firstChar == '\\')
      {
        ...
        if (ReadNextChar() != 'u')
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid escape sequence in identifier.", this.m_lineNumber, this.Source.Path);
        firstChar = ReadHexEscapeSequence(4);
        if (IsIdentifierChar(firstChar) == false)
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid character in identifier.", this.m_lineNumber, this.Source.Path);
      }
      ...
      return KeywordToken.FromString(name.ToString(), this.m_engine.CompatibilityMode, this.StrictMode);
    }
    ...
    private class LexerReader : TextReader
    {
      ...
      {
        return this.m_lexer.ReadNextChar();
      }

      public override int Peek()
      {
        return this.m_lexer.m_reader.Peek();
      }
    }
    ...
    private Token ReadNumericLiteral(int firstChar)
    {
      ...
      double result = NumberParser.ParseCore(reader, (char)firstChar, out status, true, true);
      ...
      switch (status)
      {
        ...
        case NumberParser.ParseCoreStatus.NoExponent:
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid number.", this.m_lineNumber, this.Source.Path);
        case NumberParser.ParseCoreStatus.InvalidHexLiteral:
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid hexidecimal constant.", this.m_lineNumber, this.Source.Path);
        case NumberParser.ParseCoreStatus.OctalLiteral:
         // Octal number are not supported in strict mode.
          if (this.StrictMode)
            throw new JavaScriptException(this.m_engine, "SyntaxError", "Octal numbers are not allowed in strict mode.", this.m_lineNumber, this.Source.Path);
          break;
        case NumberParser.ParseCoreStatus.InvalidOctalLiteral:
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid octal constant.", this.m_lineNumber, this.Source.Path);
      }

      // Return the result as an integer if possible, otherwise return it as a double.
      return Math.Abs(result - (int)result) < Double.Epsilon
        ? new LiteralToken((int)result)
        : new LiteralToken(result);
    }
    ...
    private Token ReadStringLiteral(int firstChar)
    {
      ...
      while (true)
      {
        ...
        if (c == firstChar)
          break;
        if (c == -1)
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected end of input in string literal.", this.m_lineNumber, this.Source.Path);
        if (IsLineTerminator(c))
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected line terminator in string literal.", this.m_lineNumber, this.Source.Path);
        if (c == '\\')
        {
          ...
          else
          {
            ...
            switch (c)
            {
              ...
              case '8':
              case '9':
                throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid octal escape sequence.", this.m_lineNumber, this.Source.Path);
              default:
    ...
    private char ReadHexEscapeSequence(int digitCount)
    {
      ...
      for (int i = 0; i < digitCount; i++)
      {
        ...
        if (IsHexDigit(c) == false)
          throw new JavaScriptException(this.m_engine, "SyntaxError", string.Format("Invalid hex digit '{0}' in escape sequence.", (char)c), this.m_lineNumber, this.Source.Path);
      }
    ...
    private char ReadOctalEscapeSequence(int firstDigit)
    {
      ...
      if (this.StrictMode)
        throw new JavaScriptException(this.m_engine, "SyntaxError", "Octal escape sequences are not allowed in strict mode.", this.m_lineNumber, this.Source.Path);

      var numericValue = firstDigit;
      for (var i = 0; i < 2; i++)
      {
        var c = this.m_reader.Peek();
        if (c < '0' || c > '9')
          break;
        if (c == '8' || c == '9')
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid octal escape sequence.", this.m_lineNumber, this.Source.Path);
    ...
    private Token ReadMultiLineComment()
    {
      ...
      int c1 = ReadNextChar();
      if (c1 == -1)
        throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected end of input in multi-line comment.", this.m_lineNumber, this.Source.Path);
    ...
    private Token ReadDivideCommentOrRegularExpression()
    {
      ...
      switch (this.ParserExpressionState)
      {
        ...
        default:
          ...
          isDivisionOperator =
            this.m_lastSignificantToken is IdentifierToken ||
            this.m_lastSignificantToken is LiteralToken ||
            this.m_lastSignificantToken == PunctuatorToken.RightParenthesis ||
            this.m_lastSignificantToken == PunctuatorToken.Increment ||
            this.m_lastSignificantToken == PunctuatorToken.Decrement ||
            this.m_lastSignificantToken == PunctuatorToken.RightBracket ||
            this.m_lastSignificantToken == PunctuatorToken.RightBrace;
          break;
      }
    ...
    private Token ReadRegularExpression()
    {
      ...
      while (true)
      {
        ...
        // checks above.
        if (IsLineTerminator(c))
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected line terminator in regular expression literal.", this.m_lineNumber, this.Source.Path);
        
        if (c == -1)
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected end of input in regular expression literal.", this.m_lineNumber, this.Source.Path);
