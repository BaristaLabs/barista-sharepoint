namespace Barista.Jurassic.Compiler
{
  ...
  public abstract class Scope
  {
    ...
    private readonly Dictionary<string, DeclaredVariable> m_variables;
    ...
    protected Scope(Scope parentScope, int declaredVariableCount)
    {
      ...
      this.m_variables = new Dictionary<string, DeclaredVariable>(declaredVariableCount);
    ...
    {
      get { return this.m_variables.Count; }
    }
    ...
    internal IEnumerable<string> DeclaredVariableNames
    {
      get
      {
        var declaredVariables = new List<DeclaredVariable>(this.m_variables.Values);
    ...
    {
      get { return this.m_variables.Values; }
    }
    ...
    internal DeclaredVariable GetDeclaredVariable(string variableName)
    {
      ...
      if (this.m_variables.TryGetValue(variableName, out variable) == false)
        return null;
      return variable;
    }
    ...
    internal virtual DeclaredVariable DeclareVariable(string name, Expression valueAtTopOfScope, bool writable, bool deletable)
    {
      if (name == null)
      ...
      DeclaredVariable variable;
      this.m_variables.TryGetValue(name, out variable);
      if (variable == null)
      {
        // This is a local variable that has not been declared before.
        variable = new DeclaredVariable { Scope = this, Index = this.m_variables.Count, Name = name, Writable = writable, Deletable = deletable };
        this.m_variables.Add(name, variable);
      }
