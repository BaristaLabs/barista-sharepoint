namespace Barista.Jurassic.Compiler
{
  ...
  internal abstract class LoopStatement : Statement
  {
    ...
    protected LoopStatement(IList<string> labels)
      : base(labels)
    {
    }
    ...
    {
      get { return (this.InitStatement is ExpressionStatement) ? ((ExpressionStatement)this.InitStatement).Expression : null; }
    }
    ...
    private Dictionary<Scope.DeclaredVariable, InferredTypeInfo> FindTypedVariables()
    {
          ...
          this.InitVarStatement.Declarations[0].InitExpression.ResultType == PrimitiveType.Int32)
      {
        ...
        initIsOkay = loopVariableScope is DeclarativeScope && loopVariableScope.HasDeclaredVariable(loopVariableName);
      }
      else if (this.InitExpression is AssignmentExpression &&
      ...
      if (everythingIsOkay)
      {
        ...
        FindTypedVariables(this.Body, result, false, ref continueEncountered);
      }
      else
      {
        // Unoptimized.
        FindTypedVariables(this, result, false, ref continueEncountered);
      }
