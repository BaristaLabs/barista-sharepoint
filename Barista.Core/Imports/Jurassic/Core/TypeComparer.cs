  ...
  public static class TypeComparer
  {
    ...
    public new static bool Equals(object x, object y)
    {
      ...
      if (TypeUtilities.IsNumeric(x) && TypeUtilities.IsString(y))
        return Math.Abs(TypeConverter.ToNumber(x) - TypeConverter.ToNumber(y)) < Double.Epsilon;
    ...
    public static bool SameValue(object x, object y)
    {
      ...
      if (y is uint)
        y = (double)(uint)y;
      if (x is double && Math.Abs((double)x - 0.0) < Double.Epsilon && y is double && Math.Abs((double)y - 0.0) < Double.Epsilon)
        if (Math.Abs((1 / (double)x) - (1 / (double)y)) > Double.Epsilon)
          return false;
      if (x is ConcatenatedString)
        x = x.ToString();
      if (y is ConcatenatedString)
