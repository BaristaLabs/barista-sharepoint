namespace Jurassic.Library
{
  ...
  public class ArrayInstance : ObjectInstance
  {
    ...
    private object[] m_dense;

    // The dense array might have holes within the range 0..length-1.
    private bool m_denseMayContainHoles;

    // The array, if it is sparse.
    private SparseArray m_sparse;
      ...
      : base(prototype)
    {
      ...
      if (length <= 1000)
      {
        this.m_dense = new object[(int)capacity];
        this.m_denseMayContainHoles = length > 0;
      }
      else
      {
        this.m_sparse = new SparseArray();
      }

      // Create a fake property for length plus initialize the real length property.
      this.m_length = length;
      FastSetProperty("length", -1, PropertyAttributes.Writable | PropertyAttributes.IsLengthProperty, false);
    }
      ...
      : base(prototype)
    {
      ...
      this.m_dense = elements;

      this.m_denseMayContainHoles = Array.IndexOf(elements, null) >= 0;

      // Create a fake property for length plus initialize the real length property.
      this.m_length = (uint)elements.Length;
      FastSetProperty("length", -1, PropertyAttributes.Writable | PropertyAttributes.IsLengthProperty, false);
    }
    ...
    public uint Length
    {
      ...
      set
      {
        uint previousLength = this.m_length;
        this.m_length = value;

        if (this.m_dense != null)
        {
          if (this.m_length < this.m_dense.Length / 2 && this.m_dense.Length > 100)
          {
            // Shrink the array.
            ResizeDenseArray(this.m_length + 10, this.m_length);
          }
          else if (this.m_length > this.m_dense.Length + 10)
          {
            // Switch to a sparse array.
            this.m_sparse = SparseArray.FromDenseArray(this.m_dense, (int)previousLength);
            this.m_dense = null;
          }
          else if (this.m_length > this.m_dense.Length)
          {
            // Enlarge the array.
            ResizeDenseArray(this.m_length + 10, previousLength);
            this.m_denseMayContainHoles = true;
          }
          else if (this.m_length > previousLength)
          {
            // Increasing the length property creates an array with holes.
            this.m_denseMayContainHoles = true;

            // Remove all the elements with indices >= length.
            Array.Clear(this.m_dense, (int)previousLength, (int)(this.m_length - previousLength));
          }
        }
        else
        {
          // Remove all the elements with indices >= length.
          if (this.m_length < previousLength)
            this.m_sparse.DeleteRange(this.m_length, previousLength - this.m_length);
        }
      }
    }
    ...
    public IEnumerable<object> ElementValues
    {
      get
      {
        ...
        if (this.m_dense != null)
        {
          for (uint i = 0; i < this.m_dense.Length; i++)
          {
            object elementValue = this.m_dense[i];
        ...
        else
        {
          ...
          foreach (var range in this.m_sparse.Ranges)
          {
    ...
    public override PropertyDescriptor GetOwnPropertyDescriptor(uint index)
    {
      if (this.m_dense != null)
      {
        ...
        if (index < this.m_length)
          return new PropertyDescriptor(this.m_dense[index], PropertyAttributes.FullAccess);
        return PropertyDescriptor.Undefined;
      }

      // The array is sparse and therefore has "holes".
      return new PropertyDescriptor(this.m_sparse[index], PropertyAttributes.FullAccess);
    }
    ...
    public override void SetPropertyValue(uint index, object value, bool throwOnError)
    {
      ...
      if (this.m_dense != null)
      {
        if (index < this.m_length)
        {
          ...
          this.m_dense[index] = value;
        }
        else if (index < this.m_dense.Length)
        {
          // The index is outside the bounds of the array but inside the allocated buffer.
          this.m_dense[index] = value;
          this.m_denseMayContainHoles = this.m_denseMayContainHoles || index > this.m_length;
          this.m_length = index + 1;
        }
        else
        {
          // The index is out of range - either enlarge the array or switch to sparse.
          if (index < this.m_dense.Length + 10)
          {
            // Enlarge the dense array.
            ResizeDenseArray((uint)(this.m_dense.Length * 2 + 10), this.m_length);
          ...
          {
            // Switch to a sparse array.
            this.m_sparse = SparseArray.FromDenseArray(this.m_dense, (int)this.m_length);
            this.m_dense = null;
            this.m_sparse[index] = value;
          }
      ...
      else
      {
        ...
        this.m_sparse[index] = value;
        this.m_length = Math.Max(this.m_length, index + 1);
      }
    }
    ...
    public override bool Delete(uint index, bool throwOnError)
    {
      if (this.m_dense != null)
      {
        this.m_dense[index] = null;
        this.m_denseMayContainHoles = true;
      }
      else
        this.m_sparse.Delete(index);
      return true;
    }
    ...
    public override bool DefineProperty(string propertyName, PropertyDescriptor descriptor, bool throwOnError)
    {
      ...
      if (arrayIndex != uint.MaxValue)
      {
        ...
        {
          if (throwOnError)
            throw new JavaScriptException(this.Engine, "TypeError", string.Format("Accessors are not supported for array elements. {0}", propertyName));
          return false;
        }
        ...
        {
          if (throwOnError)
            throw new JavaScriptException(this.Engine, "TypeError", string.Format("Non-accessible array elements are not supported. {0}", propertyName));
          return false;
        }
        ...
        if (this.m_dense != null)
          this.m_dense[arrayIndex] = value;
        else
          this.m_sparse[arrayIndex] = value;
        return true;
      }
    ...
    public override IEnumerable<PropertyNameAndValue> Properties
    {
      get
      {
        if (this.m_dense != null)
        {
          ...
          for (uint i = 0; i < this.m_dense.Length; i++)
          {
            object arrayElementValue = this.m_dense[i];
            if (arrayElementValue != null)
              yield return new PropertyNameAndValue(i.ToString(CultureInfo.InvariantCulture), new PropertyDescriptor(arrayElementValue, PropertyAttributes.FullAccess));
          }
        }
        else
        {
          ...
          foreach (var range in this.m_sparse.Ranges)
            ...
            {
              object arrayElementValue = this.m_sparse[i];
              if (arrayElementValue != null)
                yield return new PropertyNameAndValue(i.ToString(CultureInfo.InvariantCulture), new PropertyDescriptor(arrayElementValue, PropertyAttributes.FullAccess));
            }
        }
    ...
    public static ArrayInstance Concat(ObjectInstance thisObj, params object[] items)
    {
      ...
      foreach (object item in items)
        if (item is ArrayInstance)
        {
          ...
          if (((ArrayInstance)item).m_dense == null)
            dense = false;
        }
      ...
      if (dense)
      {
        ...
        foreach (object item in items)
        {
          if (item is ArrayInstance)
          {
            ...
            Array.Copy(array.m_dense, 0, result, index, (int)array.Length);
            if (array.m_denseMayContainHoles && array.Prototype != null)
            {
              // Populate holes from the prototype.
              for (uint i = 0; i < array.m_length; i++)
                if (array.m_dense[i] == null)
      ...
      else
      {
        ...
        foreach (object item in items)
        {
          if (item is ArrayInstance)
          {
            // Add the items in the array to the end of the resulting array.
            var array = (ArrayInstance)item;
            if (array.m_dense != null)
            {
              result.CopyTo(array.m_dense, (uint)index, (int)array.Length);
            ...
            {
              result.CopyTo(array.m_sparse, (uint)index);
              if (array.Prototype != null)
              {
                // Populate holes from the prototype.
                for (uint i = 0; i < array.Length; i++)
                  if (array.m_sparse[i] == null)
                    result[(uint)index + i] = array.Prototype.GetPropertyValue(i);
              }
            }
            index += (int)array.Length;
    ...
    /// </summary>
    /// <param name="thisObj"> The array that is being operated on. </param>
    /// <param name="separatorArg"> The string to use as a separator. </param>
    ...
    public static string Join(ObjectInstance thisObj, [DefaultParameterValue(",")] object separatorArg)
    {
      ...
      var separator = JurassicHelper.GetTypedArgumentValue(thisObj.Engine, separatorArg, ",");
    ...
    public object Pop()
    {
      ...
      {
        // Get the last value.
        var result = this.m_dense[this.m_length];

        // If the element does not exist in this array, it may exist in the prototype.
        if (result == null && this.Prototype != null)
          result = this.Prototype.GetPropertyValue(this.m_length);

        // Delete it from the array.
        this.m_dense[this.m_length] = null;

        // Check if the array should be shrunk.
        if (this.m_length < this.m_dense.Length / 2 && this.m_length > 10)
          ResizeDenseArray((uint)(this.m_dense.Length / 2 + 10), this.m_length);
      ...
      {
        // Get the last value.
        var result = this.m_sparse[this.m_length];
        ...
        // Delete it from the array.
        this.m_sparse.Delete(this.m_length);
    ...
    public static double Push([JSParameter(JSParameterFlags.Mutated)] ObjectInstance thisObj, params object[] items)
    {
      ...
      if (arrayLength > uint.MaxValue - items.Length)
      {
        ...
        foreach (object t in items)
        {
          ...
          thisObj.SetPropertyValue((arrayLength2++).ToString(CultureInfo.InvariantCulture), t, true);
        }
      ...
      {
        // Append the new item to the array.
        thisObj.SetPropertyValue(arrayLength++, t, true);
      }
    ...
    {
      if (this.m_length == uint.MaxValue)
      {
        // Even though attempting to push more items than can fit in the array raises an
        // error, the items are still pushed correctly (but the length is stuck at the
        // maximum).
        SetPropertyValue(this.m_length.ToString(CultureInfo.InvariantCulture), item, false);
      ...
      if (this.m_dense != null)
      {
        ...
        if (this.m_length == this.m_dense.Length)
          ResizeDenseArray(this.m_length * 2 + 10, this.m_length);

        // Append the new item to the array.
        this.m_dense[this.m_length++] = item;
      }
      else
      {
        // Append the new item to the array.
        this.m_sparse[this.m_length++] = item;
      }
    ...
    /// <param name="endArg"> The index of the element just past the last element in the section.
    /// If this value is negative it is treated as an offset from the end of the array.  If
    /// <paramref name="endArg"/> is less than or equal to <paramref name="start"/> then an empty
    ...
    public static ArrayInstance Slice(ObjectInstance thisObj, int start, [DefaultParameterValue(int.MaxValue)] object endArg)
    {
      // Get the length of the array.
      uint arrayLength = GetLength(thisObj);
      ...
      var end = JurassicHelper.GetTypedArgumentValue(thisObj.Engine, endArg, int.MaxValue);
    ...
    /// </summary>
    /// <param name="thisObj"> The array that is being operated on. </param>
    /// <param name="comparisonFunctionArg"> A function which determines the order of the
    ...
    public static ObjectInstance Sort([JSParameter(JSParameterFlags.Mutated)] ObjectInstance thisObj, [DefaultParameterValue(null)] object comparisonFunctionArg)
    {
      // Get the length of the array.
      uint arrayLength = GetLength(thisObj);
      ...
      var comparisonFunction = JurassicHelper.GetTypedArgumentValue<FunctionInstance>(thisObj.Engine, comparisonFunctionArg, null);
    ...
    public static ArrayInstance Splice([JSParameter(JSParameterFlags.Mutated)] ObjectInstance thisObj, int start, int deleteCount, params object[] items)
    {
      ...
      // Fix the arguments so they are positive and within the bounds of the array.
      start = start < 0
        ? Math.Max((int)arrayLength + start, 0)
        : Math.Min(start, (int)arrayLength);
    ...
    public static uint Unshift([JSParameter(JSParameterFlags.Mutated)] ObjectInstance thisObj, params object[] items)
    {
      ...
      if (array != null && array.m_dense != null)
      {
        // Dense arrays are supported up to 2^32-1.
        if (array.m_length + items.Length > int.MaxValue)
          throw new JavaScriptException(thisObj.Engine, "RangeError", "Invalid array length");
        ...
        if (array.m_denseMayContainHoles && array.Prototype != null)
        {
          ...
          for (uint i = 0; i < array.m_length; i++)
            if (array.m_dense[i] == null)
              array.m_dense[i] = array.Prototype.GetPropertyValue(i);
        }

        // Allocate some more space if required.
        if (array.m_length + items.Length > array.m_dense.Length)
          array.ResizeDenseArray((uint)Math.Max(array.m_dense.Length * 2 + 10, array.m_length + items.Length * 10), array.m_length);

        // Shift all the items up.
        Array.Copy(array.m_dense, 0, array.m_dense, items.Length, (int)array.m_length);

        // Prepend the new items.
        for (int i = 0; i < items.Length; i++)
          array.m_dense[i] = items[i];

        // Update the length property.
        array.m_length += (uint)items.Length;
    ...
    public static string ToString(ObjectInstance thisObj)
    {
      ...
      object result;
      if (thisObj.TryCallMemberFunction(out result, "join"))
    ...
    /// <paramref name="fromIndexArg"/>.
    /// </summary>
    /// <param name="thisObj"> The array that is being operated on. </param>
    /// <param name="searchElement"> The value to search for. </param>
    /// <param name="fromIndexArg"> The array index to start searching. </param>
    ...
    public static int IndexOf(ObjectInstance thisObj, object searchElement, [DefaultParameterValue(0)] object fromIndexArg)
    {
      // Get the length of the array.
      uint arrayLength = GetLength(thisObj);
      ...
      var fromIndex = JurassicHelper.GetTypedArgumentValue(thisObj.Engine, fromIndexArg, 0);
      ...
      for (int i = Math.Max(fromIndex, 0); i < arrayLength; i++)
      {
        ...
        if (elementValue != null && TypeComparer.StrictEquals(searchElement, elementValue))
          return i;
      }
    ...
    /// <param name="contextArg"> The value of <c>this</c> in the context of the callback function. </param>
    ...
    public static bool Every(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object contextArg)
    {
      ...
      var context = JurassicHelper.GetTypedArgumentValue<ObjectInstance>(thisObj.Engine, contextArg, null);
    ...
    public static bool Some(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object contextArg)
    {
      // callbackFunction must be a valid function.
      if (callbackFunction == null)
        throw new JavaScriptException(thisObj.Engine, "TypeError", "Invalid callback function");
      ...
      for (int i = 0; i < arrayLength; i++)
      {
        ...
        // Call the callback function.
        if (TypeConverter.ToBoolean(callbackFunction.CallFromNative("some", context, elementValue, i, thisObj)))
          return true;
      }
      return false;
    }
    ...
    public static void ForEach(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object contextArg)
    {
      // callbackFunction must be a valid function.
      if (callbackFunction == null)
        throw new JavaScriptException(thisObj.Engine, "TypeError", "Invalid callback function");
    ...
    public static ArrayInstance Map(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object contextArg)
    {
      // callbackFunction must be a valid function.
      if (callbackFunction == null)
        throw new JavaScriptException(thisObj.Engine, "TypeError", "Invalid callback function");
    ...
    public static ArrayInstance Filter(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object contextArg)
    {
      // callbackFunction must be a valid function.
      if (callbackFunction == null)
        throw new JavaScriptException(thisObj.Engine, "TypeError", "Invalid callback function");
    ...
    /// <param name="initialValueArg"> The initial accumulated value. </param>
    ...
    public static object Reduce(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object initialValueArg)
    {
      // callbackFunction must be a valid function.
      if (callbackFunction == null)
        throw new JavaScriptException(thisObj.Engine, "TypeError", "Invalid callback function");
      ...
      int i = 0;
      object accumulatedValue = JurassicHelper.GetTypedArgumentValue<object>(thisObj.Engine, initialValueArg, null);
    ...
    public static object ReduceRight(ObjectInstance thisObj, FunctionInstance callbackFunction, [DefaultParameterValue(null)] object initialValueArg)
    {
      // callbackFunction must be a valid function.
      if (callbackFunction == null)
        throw new JavaScriptException(thisObj.Engine, "TypeError", "Invalid callback function");
    ...
    {
      if (thisObj is ArrayInstance)
        return ((ArrayInstance)thisObj).m_length;
    ...
    private void ResizeDenseArray(uint newCapacity, uint length)
    {
      ...
      Array.Copy(this.m_dense, resizedArray, (int)length);
      this.m_dense = resizedArray;
    }
