namespace Barista.Jurassic.Library
{
  ...
  {
    private readonly ClrBinder m_constructBinder;
    ...
    public static ClrStaticTypeWrapper FromCache(ScriptEngine engine, Type type)
    {
      ...
      if (engine.StaticTypeWrapperCache.TryGetValue(type, out cachedInstance))
      ...
      : base(engine, GetPrototypeObject(engine, type))
    {
      ...
      if (constructors.Length > 0)
        this.m_constructBinder = new ClrBinder(constructors);
      else
      {
        ...
        {
          case TypeCode.Int32:
            this.m_constructBinder = new ClrBinder(ReflectionHelpers.Convert_ToInt32_Double.Value);
            break;
        }
      }

      this.FastSetProperty("name", type.Name, PropertyAttributes.Sealed, false);
      if (this.m_constructBinder != null)
        this.FastSetProperty("length", this.m_constructBinder.FunctionLength, PropertyAttributes.Sealed, false);
    ...
    public override ObjectInstance ConstructLateBound(params object[] argumentValues)
    {
      ...
      var result = this.m_constructBinder.Call(this.Engine, this, argumentValues);
