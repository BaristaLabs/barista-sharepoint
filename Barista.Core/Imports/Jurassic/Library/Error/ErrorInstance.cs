namespace Barista.Jurassic.Library
{
  ...
  public class ErrorInstance : ObjectInstance
  {
    ...
    {
      if (name != null)
        this.FastSetProperty("name", name, PropertyAttributes.FullAccess, false);
      if (message != null)
        this.FastSetProperty("message", message, PropertyAttributes.FullAccess, false);
    }
    ...
    internal void SetStackTrace(string path, string function, int line)
    {
      ...
      this.FastSetProperty("stack", stackTrace, PropertyAttributes.FullAccess, false);
    }
    ...
    public string ToStringJS()
    {
      ...
      return string.IsNullOrEmpty(this.Name)
        ? this.Message
        : string.Format("{0}: {1}", this.Name, this.Message);
    }
  }
}
