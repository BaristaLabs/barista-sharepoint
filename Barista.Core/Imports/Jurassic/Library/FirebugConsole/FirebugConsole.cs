namespace Barista.Jurassic.Library
{
  ...
  {
    private IFirebugConsoleOutput m_output;
    private Dictionary<string, Stopwatch> m_timers;
    ...
    {
      this.m_output.StartGroup(Format(items), false);
    }
    ...
    {
      this.m_output.StartGroup(Format(items), true);
    }
    ...
    /// </summary>
    /// <param name="nameArg"> The name of the time to create. </param>
    ...
    /// </summary>
    /// <param name="nameArg"> The name of the timer to stop. </param>
    ...
    {
      this.m_output.Log(style, FormatObjects(items));
    }
    ...
    private static string Format(object[] items)
    {
      ...
      {
        result.Append(' ');
        result.Append(TypeConverter.ToString(t));
      }
    ...
    private static object[] FormatObjects(object[] items)
    {
      ...
      int itemsConsumed = 1;
      var formatString = items[0] as string;
      if (formatString!= null)
      {
        var previousPatternIndex = 0;
        while (items.Length > itemsConsumed)
        {
          // Find a percent sign.
          int patternIndex = formatString.IndexOf('%', previousPatternIndex);
          ...
          switch (patternType)
          {
            ...
            case 'i':
              ...
              replacement = (number >= 0 ? Math.Floor(number) : Math.Ceiling(number)).ToString(CultureInfo.InvariantCulture);
              break;
            case 'f':
              replacement = TypeConverter.ToNumber(items[itemsConsumed++]).ToString(CultureInfo.InvariantCulture);
              break;
            case '%':
              replacement = "%";
              break;
