  ...
  internal sealed class JSONLexer
  {
      ...
      else
      {
        switch (c)
        {
          ...
          case -1:
            ...
            return null;
          default:
            throw new JavaScriptException(this.m_engine, "SyntaxError", string.Format("Unexpected character '{0}'.", (char)c));
        }
      }
    }
    ...
    private Token ReadKeyword(int firstChar)
    {
      ...
      if (keyword == "null")
        ...
        throw new JavaScriptException(this.m_engine, "SyntaxError", string.Format("Unexpected keyword '{0}'", keyword));
    }
    ...
    private Token ReadNumericLiteral(int firstChar)
    {
      ...
      {
        negative = true;
        firstChar = this.m_reader.Read();

        // If the first character is '-' then a digit must be the next character.
        if (firstChar < '0' || firstChar > '9')
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Invalid number.");
      }

      NumberParser.ParseCoreStatus status;
      double result = NumberParser.ParseCore(this.m_reader, (char)firstChar, out status, true, true);
      ...
      switch (status)
      {
        ...
        case NumberParser.ParseCoreStatus.InvalidHexLiteral:
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Hexidecimal literals are not supported in JSON.");
        case NumberParser.ParseCoreStatus.OctalLiteral:
        case NumberParser.ParseCoreStatus.InvalidOctalLiteral:
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Octal literals are not supported in JSON.");
      }
    ...
    private Token ReadStringLiteral(int firstChar)
    {
      ...
      while (true)
      {
        ...
        if (c == '"')
          break;
        if (c == -1)
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected end of input in string literal");
        if (c < 0x20)
          throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected character in string literal");
        ...
        if (c == '\\')
        {
          ...
          switch (c)
          {
            ...
            case 'u':
              ...
              break;
            default:
              throw new JavaScriptException(this.m_engine, "SyntaxError", "Unexpected character in escape sequence.");
          }
        }
        else
        {
    ...
    private char ReadHexNumber(int digitCount)
    {
      ...
      for (int i = 0; i < digitCount; i++)
      {
        ...
        if (IsHexDigit(c) == false)
          throw new JavaScriptException(this.m_engine, "SyntaxError", string.Format("Invalid hex digit '{0}' in escape sequence.", (char)c));
      }
