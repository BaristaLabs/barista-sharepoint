namespace Barista.Jurassic.Library
{
  ...
  {
    private Regex m_value;
    private bool m_globalSearch;
    ...
    internal RegExpInstance(ObjectInstance prototype, string pattern, string flags)
      : base(prototype)
    {
      if (pattern == null)
      ...
      try
      {
        this.m_value = new Regex(pattern, ParseFlags(flags));
      }
      ...
      this.FastSetProperty("lastIndex", 0.0, PropertyAttributes.Writable, false);
    }
      ...
      : base(prototype)
    {
      ...
      this.m_value = existingInstance.m_value;
      this.m_globalSearch = existingInstance.m_globalSearch;

      // Initialize the javascript properties.
      this.FastSetProperty("source", existingInstance.Source, PropertyAttributes.Sealed, false);
      this.FastSetProperty("global", existingInstance.Global, PropertyAttributes.Sealed, false);
      this.FastSetProperty("multiline", existingInstance.Multiline, PropertyAttributes.Sealed, false);
      this.FastSetProperty("ignoreCase", existingInstance.IgnoreCase, PropertyAttributes.Sealed, false);
      this.FastSetProperty("lastIndex", 0.0, PropertyAttributes.Writable, false);
    }
    ...
    public bool Global
    {
      get { return this.m_globalSearch; }
    }
    ...
    {
      get { return (this.m_value.Options & RegexOptions.Multiline) != 0; }
    }
    ...
    {
      get { return (this.m_value.Options & RegexOptions.IgnoreCase) != 0; }
    }
    ...
    public bool Test(string input)
    {
      ...
      var match = this.m_value.Match(input, CalculateStartPosition(input));
    ...
    public object Match(string input)
    {
      ...
      var matches = this.m_value.Matches(input);
      if (matches.Count == 0)
        return Null.Value;
    ...
    public string Replace(string input, string replaceText)
    {
      ...
      string result = this.m_value.Replace(input, match =>
      {
        lastMatch = match;
    ...
    {
      // Perform the regular expression matching.
      var match = this.m_value.Match(input);
