namespace Jurassic.Library
{
  ...
  public class StringInstance : ObjectInstance
  {
      ...
      : base(prototype)
    {
      ...
      this.m_value = value;
      this.FastSetProperty("length", value.Length, PropertyAttributes.Sealed, false);
    }
    ...
    public int Length
    {
      get { return this.m_value.Length; }
    }
    ...
    public override PropertyDescriptor GetOwnPropertyDescriptor(uint index)
    {
      ...
      {
        var result = this.m_value[(int)index].ToString(CultureInfo.InvariantCulture);
    ...
    public override IEnumerable<PropertyNameAndValue> Properties
    {
      get
      {
        ...
        for (int i = 0; i < this.m_value.Length; i++)
          yield return new PropertyNameAndValue(i.ToString(CultureInfo.InvariantCulture),
              new PropertyDescriptor(this.m_value[i].ToString(CultureInfo.InvariantCulture), PropertyAttributes.Enumerable));
    ...
    public static string CharAt(string thisObject, int index)
    {
      if (index < 0 || index >= thisObject.Length)
        return string.Empty;
      return thisObject[index].ToString(CultureInfo.InvariantCulture);
    }
    ...
    /// <param name="startIndexArg"> The character position to start searching from.  Defaults to 0. </param>
    ...
    public static int IndexOf(string thisObject, string substring, [DefaultParameterValue(0)] object startIndexArg)
    {
      var startIndex = 0;
      if (startIndexArg != null && startIndexArg != Undefined.Value)
        startIndex = TypeConverter.ToInteger(startIndexArg);
    ...
    /// <param name="thisObject"></param>
    /// <param name="substring"> The substring to search for. </param>
    /// <param name="startIndexArg"> The index of the character to start searching. </param>
    ...
    public static int LastIndexOf(string thisObject, string substring, [DefaultParameterValue(double.NaN)] object startIndexArg)
    {
      var startIndex = double.NaN;
      if (startIndexArg != null && startIndexArg != Undefined.Value)
        startIndex = TypeConverter.ToNumber(startIndexArg);
    ...
    public static object Match(ScriptEngine engine, string thisObject, object substrOrRegExp)
    {
      ...
      if (TypeUtilities.IsUndefined(substrOrRegExp))
        ...
        return engine.RegExp.Construct("", null).Match(thisObject);

      // substrOrRegExp is a string (or convertible to a string).
      return engine.RegExp.Construct(TypeConverter.ToString(substrOrRegExp), null).Match(thisObject);
    }
    ...
    /// <param name="endArg"> The character position to stop extacting. </param>
    /// <returns> A section of the string. </returns>
    [JSInternalFunction(Name = "slice", Flags = JSFunctionFlags.HasThisObject)]
    public static string Slice(string thisObject, int start, [DefaultParameterValue(int.MaxValue)] object endArg)
    {
      var end = int.MaxValue;
      if (endArg != null && endArg != Undefined.Value)
        end = TypeConverter.ToInteger(endArg);
    ...
    /// <param name="thisObject"></param>
    /// <param name="start"> The character position to start extracting. </param>
    /// <param name="lengthArg"> The number of characters to extract. </param>
    ...
    public static string Substr(string thisObject, int start, [DefaultParameterValue(int.MaxValue)] object lengthArg)
    {
      var length = int.MaxValue;
      if (lengthArg != null && lengthArg != Undefined.Value)
        length = TypeConverter.ToInteger(lengthArg);
    ...
    /// <param name="thisObject"></param>
    /// <param name="start"> The character position to start extracting. </param>
    /// <param name="endArg"> The character position to stop extracting. </param>
    /// <returns> A substring of this string. </returns>
    [JSInternalFunction(Name = "substring", Flags = JSFunctionFlags.HasThisObject)]
    public static string Substring(string thisObject, int start, [DefaultParameterValue(int.MaxValue)] object endArg)
    {
      var end = int.MaxValue;
      if (endArg != null && endArg != Undefined.Value)
        end = TypeConverter.ToInteger(endArg);
