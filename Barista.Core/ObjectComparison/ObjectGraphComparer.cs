namespace Barista.ObjectComparison
{
  ...
  public sealed class ObjectGraphComparer
  {
    ...
    /// </remarks>
    private ObjectGraphComparer(IEnumerable<GraphNode> visited)
    {
      this.m_probing = true;
      this.m_visited = new HashSet<GraphNode>(visited);
      this.m_mismatches = new List<ObjectComparisonMismatch>();
    }
    ...
    public bool Compare(GraphNode left, GraphNode right, out IEnumerable<ObjectComparisonMismatch> mismatches)
    {
      ...
      try
      {
        this.m_visiting = true;
        this.m_mismatches = new List<ObjectComparisonMismatch>();
        this.m_visited = new HashSet<GraphNode>();
        isMatch = CompareObjectGraphs(left, right);
        mismatches = this.m_mismatches;
      }
      finally
      {
        this.m_visiting = false;
        this.m_visited = null;
        this.m_mismatches = null;
      }
    ...
    internal bool IsVisited(GraphNode node)
    {
      ...
      return m_visited != null && m_visited.Contains(node);
    }
    ...
    private bool CompareObjectGraphs(GraphNode left, GraphNode right)
    {
      ...
      while (leftNodeGroups.Any() && rightNodeGroups.Any())
      {
        ...
        if (!FirstGroupKeysAreEqual(leftNodeGroups, rightNodeGroups, m_probing ? null : m_mismatches))
        {
          // If we are just probing for equality of nodes, we already 
          // know the answer
          if (m_probing) return false;
      ...
      while (leftIndex < left.Count)
      {
        ...
        for (int rightIndex = 0; rightIndex < right.Count; rightIndex++)
        {
          ...
          else
          {
            ...
            if (found) m_visited.UnionWith(probingComparer.m_visited);
          }
    ...
    private void AddMissingNodeMismatches(IEnumerable<GraphNode> nodes, bool rightIsMissing)
    {
      foreach (var n in nodes)
      {
        ...
        m_mismatches.Add(new ObjectComparisonMismatch(leftNode, rightNode, mismatchType));
      }
    }

    private bool? CheckNodesNotVisited(GraphNode left, GraphNode right)
    {
      var leftHasBeenVisited = m_visited.Contains(left);
      var rightHasBeenVisited = m_visited.Contains(right);
    ...
    private static bool CheckComparisonStrategiesSame(GraphNode left, GraphNode right)
    {
      ...
      return (right.ComparisonStrategy != null && left.ComparisonStrategy != null && left.ComparisonStrategy.GetType() == right.ComparisonStrategy.GetType());
    }
    ...
    private bool m_visiting;
    private HashSet<GraphNode> m_visited;
    private List<ObjectComparisonMismatch> m_mismatches;
    ...
    // That is needed for an unordered comparison, when we do not know apriori 
    // which node should match which
    private readonly bool m_probing;
