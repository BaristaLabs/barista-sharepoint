namespace Barista.WebSocket.Command
{
  ...
  internal abstract class FragmentCommand<TWebSocketSession> : CommandBase<TWebSocketSession, IWebSocketFragment>
      where TWebSocketSession : WebSocketSession<TWebSocketSession>, new()
  {
    ...
    protected byte[] GetWebSocketData(WebSocketDataFrame frame)
    {
      ...
      var data = length > 0
                      ? frame.InnerData.ToArrayData(offset, length)
                      : new byte[0];
    ...
    protected string GetWebSocketText(WebSocketDataFrame frame)
    {
      ...
      string text = length > 0
                      ? frame.InnerData.Decode(Utf8Encoding, offset, length)
                      : string.Empty;
