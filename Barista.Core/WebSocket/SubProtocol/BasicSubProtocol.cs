namespace Barista.WebSocket.SubProtocol
{
      ...
      where TWebSocketSession : WebSocketSession<TWebSocketSession>, new()
  {
    ...
    private readonly List<Assembly> m_commandAssemblies = new List<Assembly>();
    ...
    public override bool Initialize(IServerConfig config, SubProtocolConfig protocolConfig, ILog logger)
    {
      ...
      {
        if (protocolConfig.Commands.Cast<CommandConfig>()
          .Select(commandConfig => 
            commandConfig.Options.GetValue("assembly"))
          .Where(assembly => !string.IsNullOrEmpty(assembly))
          .Any(assembly => !ResolveCommmandAssembly(assembly)))
        {
          return false;
        }
      }
    ...
    private bool ResolveCommmandAssembly(string definition)
    {
      try
      {
        ...
        var assembliesList = assemblies as IList<Assembly> ?? assemblies.ToList();
        if (assembliesList.Any())
          m_commandAssemblies.AddRange(assembliesList);
