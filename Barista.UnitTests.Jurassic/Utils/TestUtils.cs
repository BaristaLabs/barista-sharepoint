...
namespace Barista.UnitTests.Jurassic
{
  ...
  public static class TestUtils
  {
    ...
    [ThreadStatic]
    private static Barista.Jurassic.ScriptEngine jurassicScriptEngine;
    ...
    public static Barista.Jurassic.CompatibilityMode CompatibilityMode
    {
    ...
    public static object Evaluate(string script)
    {
      ...
      if (Engine == JSEngine.JScript)
      {
        ...
        if (result == DBNull.Value)
          result = Barista.Jurassic.Null.Value;
        else if (result == null)
          result = Barista.Jurassic.Undefined.Value;
      }
      else
      {
        InitializeJurassic();
    ...
    private static void InitializeJurassic()
    {
      ...
      {
        jurassicScriptEngine = new Barista.Jurassic.ScriptEngine();
#if DEBUG
    ...
    public static object EvaluateExceptionType(string script)
    {
      ...
      else
      {
        ...
        catch (Barista.Jurassic.JavaScriptException ex)
        {
          return ex.Name;
        }
    ...
    public static Barista.Jurassic.Library.PropertyAttributes EvaluateAccessibility(string objectExpression, string propertyName)
    {
      ...
      var result = (Barista.Jurassic.Library.PropertyAttributes)0;
      ...
      if ((bool)Evaluate(string.Format("Object.getOwnPropertyDescriptor({0}, {1}).enumerable", objectExpression, propertyName)) == true)
        result |= Barista.Jurassic.Library.PropertyAttributes.Enumerable;

      // Check if the property is writable.
      if ((bool)Evaluate(string.Format("Object.getOwnPropertyDescriptor({0}, {1}).writable", objectExpression, propertyName)) == true)
        result |= Barista.Jurassic.Library.PropertyAttributes.Writable;
      ...
      if ((bool)Evaluate(string.Format("Object.getOwnPropertyDescriptor({0}, {1}).configurable", objectExpression, propertyName)) == true)
        result |= Barista.Jurassic.Library.PropertyAttributes.Configurable;
